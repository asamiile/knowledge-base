...
**Response:** `{ "id", "saved_search_id", "title_snapshot", "status", "imported_content", "imported_payload", "error_message", "created_at" }` — arXiv 定期実行成功時、`imported_payload` に `written`（相対パス配列）に加え `match_hints`（各パスごとの `snippet`・`matched_in`: `title` / `abstract`）が含まれる場合がある。
